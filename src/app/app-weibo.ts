...
// 9宫格列表
export async function list(res: KoaContext) {
 ...
 */
export async function get(res: KoaContext) {
    const reqQuery = res.query as { page: string | number }
 ...
 */
export async function video(res: KoaContext) {
    let json: WeiBoBlogReturn | ResData<string | null>
    const reqQuery = res.query as { since_id: string }
 ...
 * @returns 返回一个Promise，解析为处理后的响应数据。
 */
export async function detail(res: KoaContext) {
